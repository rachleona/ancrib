...
{
    protocipher.call(this, plaintext, ciphertext, options.key, info.lucifer.modes.pure )
    ...
    const makeSubKeys = (key, format='utf8') => {
        ...
        .reduce((res, a, i) => {
            ...
            if(i == 0)
            {
                const A = kp.slice(0, 3).reduce( (num, v, j) => num + v * Math.pow(256, 2 - j), 0x000000) 
                const B = kp.slice(3).reduce( (num, v, j) => num + v * Math.pow(256, 3 - j), 0x00000000)
                prev = { 
        ...
        .map( ({ C, D }) => {
            const pair = Buffer.concat([intToBytes(C * 16 + ((D & 0xf000000) >>> 24), 4), intToBytes(D & (Math.pow(2, 24) - 1), 3)])
    ...
    const cipher = (plaintext, key, rev=false, pformat='utf8', kformat='utf8', rformat='utf8') => {
        const keys = rev ? makeSubKeys(key, kformat).reverse() : makeSubKeys(key, kformat)
        ...
        const res = Buffer.concat(blocks.map( block => {
            const buf = Buffer.alloc(8)
            IPi.map( (byte, i) => {
                let num = 0x00
                byte.map( (v, j) => {
    ...
    this.encrypt = (pformat='utf8', kformat='utf8', rformat='hex') => {
        ...
        if(errors.length == 0) this.setC(cipher(p, k, 0, pformat, kformat, rformat))
    ...
    this.decrypt = (pformat='hex', kformat='utf8', rformat='utf8') => {
        ...
        if(errors.length == 0) this.setP(cipher(c, k, 1, pformat, kformat, rformat))
...
{ 
    protocipher.call(this, plaintext, ciphertext, options.key, info.lucifer.modes.triple )

    const encrypt = (plaintext, key, pformat='utf8', kformat='utf8', rformat='utf8') => {
    ...
    const decrypt = (ciphertext, key, pformat='utf8', kformat='utf8', rformat='utf8') => {
