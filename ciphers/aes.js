        ...
        for (let i = 0; i < 4; i++) {
          ...
          mCon.map((row, j) => {
            ...
            row.map((v, k) => {
              ...
              values[k] = mod
                ? ((arr[i][k] << 1) & 0xff) ^ 0x1b ^ (arr[i][k] * (v & 0x01))
                : ((arr[i][k] << 1) & 0xff) ^ (arr[i][k] * (v & 0x01))
            })
          ...
          miCon.map((row, j) => {
            ...
            row.map((v, k) => {
              if (v === 0x09) {
                ...
                values[k] =
                  multiply2(multiply2(arr[i][k], 2) ^ arr[i][k]) ^ arr[i][k]
              } else if (v === 0x0d) {
                values[k] =
                  multiply2(multiply2(arr[i][k]) ^ arr[i][k], 2) ^ arr[i][k]
              } else {
                values[k] = multiply2(
                  multiply2(multiply2(arr[i][k]) ^ arr[i][k]) ^ arr[i][k]
                )
              }
            })
            newRow[j] = values[0] ^ values[1] ^ values[2] ^ values[3]
