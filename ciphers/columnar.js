...
{
    protocipher.call(this, plaintext, ciphertext, options.key, info.columnar.modes.pure)
    
    const encrypt = (key, text) => {
        const k = key.substr(0, text.length).split("").sort().map( v => key.indexOf(v) )
        const len = k.length
...
{
    protocipher.call(this, plaintext, ciphertext, options.key, info.columnar.modes.scytale)
