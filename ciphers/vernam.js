...
function vernam(plaintext, ciphertext, options)
{
    ...
    protocipher.call(this, plaintext, ciphertext, options.key, info.vernam.modes.pure)
    ...
    const cipher = (plaintext, key, rev=false) => {
        ...
        const keys = rev ? inverseKey(key).filter( v => alphabets.includes(v) ) : key.toUpperCase().split("").filter( v => alphabets.includes(v) )
        let counter = 0
